...
#ifdef _HARMATTAN_Y
      original_view_vars.viewpitch = player->mo->angle_y;// + viewangleoffset_y;
#endif
    }
...
#ifdef _HARMATTAN_Y
    viewangle_y = original_view_vars.viewpitch + FixedMul (frac, R_SmoothPlaying_Get(player->mo->angle_y)/* + viewangleoffset_y*/ - original_view_vars.viewpitch);
#endif
  }
  else
  {
...
#ifdef _HARMATTAN_Y
    viewangle_y = R_SmoothPlaying_Get(player->mo->angle_y);
#endif
  }
}
