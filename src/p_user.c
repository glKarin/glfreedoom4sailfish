...
#ifdef _HARMATTAN_Y
  pitch = mo->angle_y + (cmd->angleturn_y << 16);
#define VIEW_PITCH_UP 3221225472U // 270
#define VIEW_PITCH_DOWN 1073741824U // 90
#define VIEW_PITCH_MID 0U // 360
#define VIEW_PITCH_MID2 (unsigned)((UINT_MAX) / 2)
	if((pitch < VIEW_PITCH_UP && pitch >= VIEW_PITCH_MID2) && mo->angle_y >= VIEW_PITCH_UP)
	{
		mo->angle_y = VIEW_PITCH_UP;
	}
	else if((pitch > VIEW_PITCH_DOWN && pitch < VIEW_PITCH_MID2) && mo->angle_y <= VIEW_PITCH_DOWN)
	{
		mo->angle_y = VIEW_PITCH_DOWN;
	}
	else
		mo->angle_y = pitch;
#endif
...
#ifdef _HARMATTAN_Y
    original_view_vars.viewpitch = R_SmoothPlaying_Get(player->mo->angle_y);// + viewangleoffset_y;
#endif
  }
