...
{
	char fmt_str[] = "[%s]: %s -> %s\n";
	lprintf( LO_INFO, fmt_str, _HARMATTAN_APPNAME, "Ver", _HARMATTAN_VER);
	lprintf( LO_INFO, fmt_str, _HARMATTAN_APPNAME, "Code", _HARMATTAN_DEVCODE);
	lprintf( LO_INFO, fmt_str, _HARMATTAN_APPNAME, "Dev", _HARMATTAN_DEV);
	lprintf( LO_INFO, fmt_str, _HARMATTAN_APPNAME, "Release", _HARMATTAN_RELEASE);
}
...
void gld_Init(int width, int height)
{
...
#endif
  {
	...
	//SDL_GL_GetDrawableSize(screen, &wlw, &wlh);

	lprintf(LO_INFO, "[Harmattan]: Initialize virtual button layer......");
	karinGLNewVKB(0, 0, 0, wlh, wlw);
	lprintf(LO_INFO, "done\n");
	lprintf(LO_INFO, "[Harmattan]: Initialize OpenGL FBO (%dx%d)......", wlw, wlh);
	fbo_object = new_karin_FrameBufferObject(width, height, wlw, wlh);
	fbo->bind(fbo);
	lprintf(LO_INFO, "done\n");
...
#ifdef _HARMATTAN_Y
  pitch = 360.0f-(float)(viewangle_y>>ANGLETOFINESHIFT)*360.0f/FINEANGLES;
	// up 270 mid 360 down 90 back 180
	/*
	printf("%f    %ud,   %ud\n", pitch, (unsigned)((int)(pitch * FINEANGLES / 360)<<ANGLETOFINESHIFT), viewangle_y);
	printf("M 360    %ud,   %ud\n", (unsigned)((360 * FINEANGLES / 360)<<ANGLETOFINESHIFT), viewangle_y);
	printf("U 270    %ud,   %ud\n", (unsigned)((90 * FINEANGLES / 360)<<ANGLETOFINESHIFT), viewangle_y);
	printf("D 90     %ud,   %ud\n", (unsigned)((270 * FINEANGLES / 360)<<ANGLETOFINESHIFT), viewangle_y);
	*/
#endif
  ...
  if (flat->sectornum>=0)
  {
    ...
    for (loopnum=0; loopnum<sectorloops[flat->sectornum].loopcount; loopnum++)
    {
      ...
      // set the mode (GL_TRIANGLES, GL_TRIANGLE_STRIP or GL_TRIANGLE_FAN)
			{
				GLfloat *harm_Data = NULL;
				glBegin(currentloop->mode)
					GLfloat *harm_Temp_Data;

					harm_Data = calloc((currentloop->vertexindex + currentloop->vertexcount - currentloop->vertexindex) * 5, sizeof(GLfloat));
					for (vertexnum=currentloop->vertexindex; vertexnum<(currentloop->vertexindex+currentloop->vertexcount); vertexnum++)
					{
						...
						harm_Temp_Data = harm_Data + (vertexnum * 5);
						glTexCoord2fv(&gld_texcoords[vertexnum*2]);
						// set vertex coordinate
						harm_Temp_Data = harm_Data + (vertexnum * 5) + 2;
						glVertex3fv(&gld_vertexes[vertexnum*3]);
					}
				glEnd()
#undef free
				free(harm_Data);
#define free(x) Z_Free(x)
			}
